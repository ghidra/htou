...
from urho_utils import FloatToString, WriteXmlFile, BinaryFileWriter
...
class Vector:
	...
	def __truediv__(self,f):
		return Vector(self.x/f,self.y/f,self.z/f,self.w/f)
	def __iadd__(self,v):
		self.x+=v.x
		self.y+=v.y
		self.z+=v.z
...
def UrhoWriteModel(model, filename):
	...
	#print len(model.vertexBuffers)
	# For each vertex buffer
	for buffer in model.vertexBuffers:
		# Vertex count
		fw.writeUInt(int(len(buffer.vertices)))
